...
<section class="bg-gray-50 lg:pt-16 lg:pb-1 py-8 relative">
        <div class="max-w-screen-xl px-4 mx-auto space-y-12 lg:space-y-20  lg:px-6">
            ...
            <div class="items-center gap-8 lg:grid lg:grid-cols-2 xl:gap-16">
                <div class="text-gray-500 sm:text-lg">
                    ...
                    <p class="mb-4 lg:text-lg">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Perspiciatis vero illum ipsum id laudantium sunt omnis sequi inventore laboriosam, officia, fuga nam tenetur? Porro, quos.</p>
        ...
        <swiper-container id="article" init="false" class="container m-auto overflow-x-hidden">
            ...
            <swiper-slide class="card">
                <img src="{{ $article->thumbnail }}" alt="" class="object-cover m-auto thumb w-80 h-44 rounded-2xl bg-white hover:shadow-xl shadow-slate-100 transition-all">
                <div class="date flex w-80 font-base m-auto mt-1 text-white space-x-2">
                    <p class="category text-white px-3 py-1 rounded-full bg-{{$article->category->color}}-600 text-xs font-medium">{{$article->category->name}}</p>
                    <p class="font-mono text-xs text-slate-900 font-light my-auto">{{ $article->updated_at->diffForHumans() }}</p>
                </div>
                <div class="title w-80 lg:w-72 m-auto mt-3 pb-3">
                    <a href="" class="inline-block text-2xl font-semibold text-slate-800 leading-tight tracking-wide">{{ $article->title }}</a>
                </div>
            </swiper-slide>
            @endforeach
        </swiper-container>
