...
@section('content')
    ...
    <section class="pt-20 md:pt-28 font-poppins relative">
        ...
        <header class="md:h-[20vh] lg:h-[50vh] w-full px-3 max-w-screen-md mx-auto flex flex-col justify-center">
            ...
            {{-- search form --}}
            <form class="flex items-center mt-5" method="get" action="/blog">
                ...
                <div class="relative w-full">
                    ...
                    <input type="text" id="simple-search"
                        ...
                        placeholder="Cari artikel" autocomplete="off" name="artikel" value="{{ old('artikel') }}">
                </div>
                <button type="submit"
        ...
        @if ($articles->count() == 0)
            <div class="flex flex-col items-center justify-center mt-20">
                <h1 class="text-2xl font-bold mt-5">Tidak ada artikel</h1>
            </div>
        @else
            {{-- jumbo section --}}
            <h3 class="font-extrabold text-2xl max-w-7xl m-auto px-3 text-blue-600 uppercase"><span
                    class="text-black underline">latest</span> post</h3>
            ...
            {{--  artikel --}}
            <section class="article mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-11/12 gap-4 m-auto">
                @foreach ($articles->skip(1) as $article)
                    <div class="card group">
                        <a href="/tulisan/{{ $article->slug }}" class="card">
                            <img src="{{ $article->thumbnail }}" alt=""
                                class="object-cover m-auto thumb w-11/12 md:w-10/12 h-48 rounded-xl bg-white group-hover:shadow-lg group-hover:shadow-slate-300 transition-all duration-300">
