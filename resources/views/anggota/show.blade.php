...
@section('content')
            ...
            @auth
                ...
                @else
                    @if ($user->id == auth()->user()->id)
                        <a href="{{route('profile.edit')}}" class="bg-slate-200 inline-block p-2 rounded-lg right-14 absolute"><i class="bi bi-pen mr-2"></i>edit</a>
                    @endif
                @endif
            @endauth
            <div class="img w-40 h-40 rounded-full overflow-hidden m-auto">
                
                <img src="@if ($user->profile) {{asset("storage/$user->profile")}} @else {{asset('img/HIMATIKA-Trilogi.png')}} @endif" class="w-full" alt="">
            </div>
    ...
    <section class="content max-w-xl m-auto p-3 @if(Request::is('dashboard')) bg-slate-100 @else bg-white mx-3 lg:mx-auto @endif md:rounded-xl tracking-wide text-lg lg:text-base">
        {!! $user->bio !!}
    </section>
@endsection